...
program ProgramTemplate
  ...
  real(kind(1d0)), allocatable   :: dEval(:),Xi(:,:),Xip(:),Bo(:,:),Xo(:,:)
  integer                        :: i,j,k,l,m,n,ia,ib,ic,id
  real(kind(1d0)), allocatable   :: WORK(:)
  integer                        :: LWORK,INFO,uid,Ndim
  integer, allocatable           :: iWORK(:),brr(:),bsnu(:),bsino(:)
  

  type(ClassBspline)             :: Bsp_in,Bsp_out,Bs_io(2)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !          write(*,*) j,l,i
  !          Xip(j)=Xip(j)+MixedBSplineIntegral(Bsp_in,Bsp_out,fptr,i,l,0,0,0.001d0,0.999d0)*Xi(l-Nf_Bs_out+1,ia)
  !       End Do
  !    End Do
...
! Now we define the orthonormalization we now is working on well

  NLAST=Nbs_out-1-(Nf_Bs_out-1)
  NLAST=NLAST  !how many do we remove?
  Ndim=Nl_Bs_in-1+NLAST
  allocate(Xo(1:Ndim,1:Ndim))
  Xo=0.d0
  Xo(Nl_Bs_in:Ndim,Nl_Bs_in:Ndim)=Xi(:,:)

  ! Do i=1,Nl_Bs_in-1
  !    Xo(i,i)=1.d0
  ! End Do
  Xo(1:Nl_Bs_in-1,1:Nl_Bs_in-1)=Bo
  
  Bs_io(1)=Bsp_in
  Bs_io(2)=Bsp_out
  allocate(bsnu(1:Ndim),bsino(1:Ndim))
  Do i=1,Nl_Bs_in-1
    bsnu(i)=i+1
    bsino(i)=1
  End Do
  Do i=Nl_Bs_in,Ndim
    bsnu(i)=i-Nl_Bs_in+Nf_Bs_out
    bsino(i)=2
 End Do
 ...
 !Ndim=Ndim-1
  ...
  Do j=1,Ndim
     Do i=1,Ndim
        ...
        Do ia=1,Ndim
           Do ib=1,Ndim
ivalue  = ivalue +Xo(ib,i)*Xo(ia,j)*MixedBSplineIntegral(Bs_io(bsino(ia)),Bs_io(bsino(ib)),fptr,bsnu(ia),bsnu(ib),0,2,0.d0,1.d0)
ivalue2 = ivalue2+Xo(ib,i)*Xo(ia,j)*MixedBSplineIntegral(Bs_io(bsino(ia)),Bs_io(bsino(ib)),fptr,bsnu(ia),bsnu(ib),0,0,0.d0,1.d0)
           End Do
        End Do
        dMat(j,i)=-0.5d0*ivalue
        dOve(j,i)=ivalue2
 ...
 INFO=0
 call dsygv(1,'V','U',Ndim,dMat,Ndim,dOve,Ndim,dEval,WORK,LWORK,INFO)
 write(*,*) "INFO eigen proyector 2",INFO
 Do i=1,Ndim
    write(*,*) i,dEval(i),0.5*(i*3.14159265359d0)**2
 End Do
 deallocate(WORK)
...
!!Orthonormalization GS
  ...
  Do j=Nl_Bs_in,Ndim
     Do i=1,Nl_Bs_in-1
        ...
        Do ia=1,Ndim
           Do ib=1,Ndim
ivalue  = ivalue +Xo(ib,i)*Xo(ia,j)*MixedBSplineIntegral(Bs_io(bsino(ia)),Bs_io(bsino(ib)),fptr,bsnu(ia),bsnu(ib),0,0,0.d0,1.d0)
           End Do
        End Do
        Xo(:,j)=Xo(:,j)-ivalue*Xo(:,i)
     End Do
