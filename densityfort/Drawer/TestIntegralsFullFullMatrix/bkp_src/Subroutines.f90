...
!  Rmax=30.1333333333333d0 
  ...
  !.. A problem appears here when running with the debug 
  write(*,*) NNodes
  write(*,*) BsplineOrder,size(bsgrid)
  write(*,*) bsgrid
...
subroutine InitBasisParameters
  ...
  End Do
  write(*,*) "NprimUkrmol",NprimUkrmol,nPWC
!pause
...
subroutine InitIelement(ielement,ielement0)
  ...
  integer           :: l,m,i,j,k,ia,ib,ic,iIrreps,ir
  ...
  allocate(absTOl(sum(nTOT+nPWC)),absTOm(sum(nTOT+nPWC)),absTOnr(sum(nTOT+nPWC)))
  j = 1
  do i = 1, nIrreps
     do k = 1, nLOC(i)
        absTOrel(j)=k
