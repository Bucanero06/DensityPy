...
!  integer, optional, intent(in) :: lll, mmm
  ...
  !do i=1,8
     write(*,*) GlobalIntegral%GetnBS()
   !  end do
  qres = 0.d0
                                                                    !optional
!GlobalIntegral%Get1B(self, OpLabel, otypes, iIrrOrbBra, iIrrOrbKet, l, m)
  ...
  if(associated(Mat)) qres = Mat%A(aia,aib)
     ...
     if(chi3.eq."SRC")then  
        do lk = 0, 2*lmax
           do mk = -lk, lk
              ...
              if(.not.GlobalIntegral%OneBIsInitialized(OpLabel,"HY_BS",iIrr3,ilmp))cycle
              
              Mat = GlobalIntegral%GetLocMoments(iIrr1,iIrr2,ilmk)
              moments = Mat%A(ai1, ai2)
              call Mat%Free()
              Mat = GlobalIntegral%Get1B(OpLabel,"HY_BS",iIrr3,ilmp)
              mult    = Mat%A(ai3, ai4)
              call Mat%Free()
...
!              write(*,*) GlobalIntegral%LocMoments%LOC_LOC(iIrr1,iIrr2,ilmk)%ISINITIALIZED(),GlobalIntegral%Int_1B(int_ID)%PWC_PWC(ilm,ilmp)%ISINITIALIZED(),iIrr1,iIrr2
              if(.not.GlobalIntegral%LocMomentsIsInit(iIrr1,iIrr2,ilmk))cycle
              if(.not.GlobalIntegral%OneBIsInitialized(OpLabel,"BS_BS",ilm,ilmp))cycle
              Mat = GlobalIntegral%GetLocMoments(iIrr1,iIrr2,ilmk)
              moments = Mat%A(ai1, ai2)
              call Mat%Free()
              Mat = GlobalIntegral%Get1B(OpLabel,"BS_BS",ilm,ilmp)
              mult    = Mat%A(ai3, ai4)
              call Mat%Free()
              ivalue  = ivalue + moments * mult
              
              ! write(*,*) moments , mult,lk,mk,ilmk,int_ID
              ! write(*,*) l,m,lp,mp
              ! write(*,*) ilm,ilmp, ai3, ai4,ind(3),ind(4)
           enddo
        enddo
     endif
     qres = ivalue
