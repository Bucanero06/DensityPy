             ...
             ! write(*,*) "ooo",ME+rvalue
              ...
              do lk = abs (l - lp ), l + lp
                 ...
                 rvalue = rvalue +  factor * &
                      GlobalIntegral%BsBlocks%GetMultipoles(nr, nrp, lk)
                 !write(*,*) "ixa",lk,ME,ME+rvalue,factor,GlobalIntegral%BsBlocks%GetMultipoles(nr, nrp, lk)
              end do
...
!  integer, optional, intent(in) :: lll, mmm
  ...
  cha   = absTOchar(il)
  chb   = absTOchar(ir)
  !write(*,*) cha,chb,iIrra,iIrrb,ia,ib,aia,aib,OpLabel!,li,mi
  qres = 0.d0
  ...
  if(abs(mi)<=li)then
     ...
     if((chb.eq."SRC").and.(cha.eq."PWC"))then
        l   = absTOl(il)
        m   = absTOm(il)
        nr  = absTOnr(il)-NfirstBspl+1
        ilm = l**2 + l + m + 1
        if(.not.GlobalIntegral%OneBIsInitialized(OpLabel,"HY_BS",iIrrb,ilm))return
        Mat => GlobalIntegral%Get1B(OpLabel,"HY_BS",iIrrb,ilm)

        !***I DON TLIKE THIS: THE MATRIX HAS NOT THE CORRECT PHYSICAL DIMENSIONS
        if(aia.gt.Mat%NColumns())return
        qres = Mat%Element(aib,aia)
        return
     endif
     if((cha.eq."PWC").and.(chb.eq."PWC"))then
        l   = absTOl(ir)
        ...
        !write(*,*) "L,M",l,m,lp,mp,OpLabel
        ilm  = l  **2 + l  + m  + 1
        ilmp = lp **2 + lp + mp + 1
        if(.not.GlobalIntegral%OneBIsInitialized(OpLabel,"BS_BS",ilmp,ilm))return

        Mat => GlobalIntegral%Get1B(OpLabel,"BS_BS",ilmp,ilm)
        !write(*,*) "ACASASA",GlobalIntegral%OneBIsInitialized(OpLabel,"BS_BS",ilmp,ilm),Mat%Element(aia,aib),qres
     endif
