...
program TestIntegrals
  ...
  integer                             :: Ndim_sym, i_sym, j_sym
  integer, allocatable                :: SymIndexes(:)
  real(kind(1d0))                     :: H_Av_Err, S_Av_Err, H_oo_Err, S_oo_Err
  ...
  call GetRunTimeParameters( ProgramInputFile, AssignedSymmetry, lswitch )
  ...
  Ndim_sym=0
  do j=1,Ndim
     if(TotSym(j) == TotIrrepIndex ) Ndim_sym=Ndim_sym+1
  enddo
  allocate(SymIndexes(Ndim_sym))
  i = 1
  do j=1,Ndim
     if(TotSym(j) == TotIrrepIndex )then
        SymIndexes(i) = j
        i = i + 1
     endif
  enddo
  ...
  allocate(Hirr(1:Ndim_sym,1:Ndim_sym),Over(1:Ndim_sym,1:Ndim_sym),En(1:Ndim_sym))  
  Over=0.d0
  Hirr=0.d0
  En=0.d0
  ...
  Do j_sym = 1, Ndim_sym
     ...
     j = SymIndexes(j_sym)
     call cpu_time(time1)
     write(*,*) "j_sym", j_sym, Ndim_sym
     
     Do i_sym = 1, Ndim_sym
        i = SymIndexes(i_sym)
        !Over(i_sym,j_sym)=1.d0*Kron(i_sym,j_sym)
        !write(*,*) "i_sym", i_sym
        ...
        Over(i_sym,j_sym) = Overlap
        Hirr(i_sym,j_sym) = OneBody + Biel + Overlap*NucRep
        if(isnan(Hirr(j_sym,i_sym)))then
           write(*,*) Over(i_sym,j_sym)
           write(*,*) i_sym,j_sym,iflag,(1.d0/dble(NRefConf))
           stop
        end if
  ...
  WORK=0.d0
  INFO=0
  call dsygv(1,'N','U',Ndim_sym,Hirr,Ndim_sym,Over,Ndim_sym,En,WORK,LWORK,INFO)
  ...
  ! !.. File for the comparision of the energies
   open(unit=2, file="energy_ukrmol_"//AssignedSymmetry//".dat")
