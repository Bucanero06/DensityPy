...
contains
    ...
    subroutine GetRunTimeParameters(input_directory, output_directory, molecular_geometry_file, &
        ...
        integer :: pos1, pos2, i, number_of_orbitals, ERROR_FLAG
        ...
        allocate(molecular_geometry_file, source = trim(adjustl(strnBuf)))
        ...
        do i = 1, counted_number_of_orbitals
            pos1 = index(orbital_string, " ")
            if(pos1>0)then
                read(orbital_string(1:pos1 - 1), *) ivorb(i)
                orbital_string = orbital_string(pos1 + 1:)
            else
                read(orbital_string, *) ivorb(i)
            endif
        enddo
