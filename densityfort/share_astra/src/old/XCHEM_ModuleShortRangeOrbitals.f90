...
contains
  ...
  subroutine ClassDiffuseOrbitalsLoad( DifSet, unit ) 
    ...
    if ( Lmax /= DifSet.Lmax ) then
       call Assert( "The maximum angular momentum read from the diffuse orbitals configuraion file differs from the value used in the diffuse orbital initialization." )
    end if
    !
    !
...
!!$       !
  ...
  function ClassDiffuseOrbitalsGetCartesianSet( DifSet ) result( CartSet )
    ...
    class(ClassDiffuseOrbitals), intent(inout) :: DifSet
    type(ClassCartesianSymmetricSet), pointer :: CartSet
    type(ClassCartesianSymmetricSet), target :: CopyCartSet
    !
    CartSet => NULL()
    CopyCartSet = DifSet.CartesianSymSet
    CartSet => CopyCartSet
    call CopyCartSet.Free()
    !
  ...
  function ClassDiffuseOrbitalsGetSpherSet( DifSet ) result( SpherSet )
    ...
    type(ClassSpherSymmetricSet), target :: CopySpherSet
    !
    SpherSet => NULL()
    CopySpherSet = DifSet.SpherSymSet
    SpherSet => CopySpherSet
    call CopySpherSet.Free()
    !
...
!!$    deallocate( OverlapMat )
  ...
  subroutine ClassShortRangeSymOrbitalsFetchklmMask( SRSOrb, NewXlmToCartMat, L, M, NumExponents )
    ...
    !
    do i = 1, Counter
       NewXlmToCartMat(:,(i-1)*NumExponents+1:i*NumExponents) = IntermediateXlmToCartMat(:NewNumRows,(AuxVec(i)-1)*NumExponents+1:AuxVec(i)*NumExponents)
    end do
    !
  ...
  subroutine ClassConditionerBlockInit( this, MethodLabel, Threshold, NumBsDropBeginning, NumBsDropEnding, Irrep, KetXlm, PerfectProjector, Block )
