...
def ParseConfigurationFile(option1, option2, option3, option4, option5, option6, option7, option8, option9, option10,
                           configuration_file):  # todo pass args as dict or list instead of 10 args (or use *args)
    ...
    if configuration_file:
        if path.exists(configuration_file):
            if not any(data):
                logger.info("Found " + str(configuration_file) + " but nothing was ran: "
                                                                 "\n   No command line  action arguments used.")
                sys.exit()
            else:
                logger.info('Found chargemigration.ini')
                logger.info(str(configuration_file))


    elif path.exists("chargemigration.ini"):
        if not any(data):
            logger.info("Found chargemigration.ini but nothing was ran: "
                        "\n   No command line  action arguments used.")
            sys.exit()
        else:
            logger.info('Found chargemigration.ini')
