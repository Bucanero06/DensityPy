...
class Orbitals(object):
    ...
    def read_h5_basis(self):
        with h5py.File(self.file, 'r') as f:
            ...
            self.centers = [{'name': str(l.decode('ascii')).strip(), 'Z': q, 'xyz': x} for l, q, x in
                            zip(labels, charges, coords)]
            ...
            bf_id = np.rec.fromrecords(np.insert(f[basis_function_ids][:], 4, -1, axis=1),
                                       names='c, s, l, m, tl')  # (center, shell, l, m, true-l)
            ...
            if (sym > 1):
                sbf_id = np.rec.fromrecords(np.insert(f['BASIS_FUNCTION_IDS'][:], 4, -1, axis=1),
                                            names='c, s, l, m, tl')
            else:
                sbf_id = bf_id
            ...
            p_shells, p0_counts = np.unique(
                [np.array(b)[['c', 's', 'tl']].copy() for b in bf_id if ((b['l'] == 1) and (b['m'] == 0))],
                return_counts=True)
            ...
            for i, c in enumerate(self.centers):
                ...
                for l in range(maxl + 1):
                    ...
                    for s in range(maxshell):
                        ...
                        ll.append([0, [pp.tolist() for p, pp in zip(prids, prims) if
                                       ((p[0] == i + 1) and (p[1] == l) and (p[2] == s + 1))]])
                    c['basis'].append(ll)
                ...
                for l in range(maxl - 1):
                    ...
                    cont = [(b['l'], b['tl'], b['s']) for b in
                            bf_id[np.logical_and(bf_id['c'] == i + 1, bf_id['l'] == l)] if (b['l'] != b['tl'])]
    ...
    def read_h5_MO(self):
        with h5py.File(self.file, 'r') as f:
            ...
            for i, b, s in zip(ii, self.N_bas, self.irrep):
                for orb, orb_a, orb_b in zip_longest(self.base_MO[0][i:i + b], self.base_MO['a'][i:i + b],
                                                     self.base_MO['b'][i:i + b]):
                    if (orb):
                        orb['sym'] = s
            ...
            if (mod == 'CASPT2'):
                ...
                if ('DENSITY_MATRIX' in f):
                    ...
                    if ('H_EFF' in f):
                        H_eff = f['H_EFF']
                        self.roots.extend([(i + 1, '{0}: {1:.6f}'.format(r, e)) for i, (r, e) in
                                           enumerate(zip(rootids, np.diag(H_eff)))])
                        self.msroots = [(0, 'Reference')]
                        self.msroots.extend(
                            [(i + 1, '{0}: {1:.6f}'.format(i + 1, e)) for i, e in enumerate(f['STATE_PT2_ENERGIES'])])
                    else:
                        self.roots.extend([(i + 1, '{0}: {1:.6f}'.format(r, e)) for i, (r, e) in
                                           enumerate(zip(rootids, f['STATE_PT2_ENERGIES']))])
                ...
                if ('DENSITY_MATRIX' in f):
                    ...
                    self.roots.extend([(i + 1, '{0}: {1:.6f}'.format(r, e)) for i, (r, e) in
                                       enumerate(zip(rootids, f['ROOT_ENERGIES']))])
            ...
            if (density == 'Difference'):
                ...
                with h5py.File(self.file, 'r') as f:
                    if (self.wf == 'SI'):
                        ...
                        if (self.sdm):
                            sdm = f['SFS_TRANSITION_SPIN_DENSITIES'][r2, r2, :] - f['SFS_TRANSITION_SPIN_DENSITIES'][r1,
                                                                                  r1, :]
                    else:
            ...
            for dm_, MO_ in zip(dmlist, MOlist):
                ...
                for s in set([o['sym'] for o in act]):
                    ...
                    for i in range(vec.shape[1]):
                        types = []
                        for a in tp_act:
                            if (sum([j ** 2 for n, j in enumerate(vec[:, i]) if
                                     (symact[n]['type'] == a)]) > mix_threshold):
                                types.append(a)
            ...
            for s in set([o['sym'] for o in act_l]):
                ...
                if (not np.allclose(symdm, 0)):
                    ...
                    for i in range(vec_l.shape[1]):
                        types = []
                        for a in tp_act:
                            if (sum([j ** 2 for n, j in enumerate(vec_l[:, i]) if
                                     (symact_l[n]['type'] == a)]) > mix_threshold):
                                types.append(a)
                        mix_l.append(types[0] if (len(types) == 1) else '?')
                    for i in range(vec_r.shape[1]):
                        types = []
                        for a in tp_act:
                            if (sum([j ** 2 for n, j in enumerate(vec_r[:, i]) if
                                     (symact_r[n]['type'] == a)]) > mix_threshold):
                                types.append(a)
                    ...
                    if (num is None):
                        ...
                        while True:
                            ...
                            try:
                                ...
                                self.centers.append({'name': l, 'Z': q, 'xyz': np.array(
                                    [fortran_float(x), fortran_float(y), fortran_float(z)]) * unit})
                                num += 1
                            except:
                                f.seek(save)
                                break
                        ...
                        for i in range(num):
                            ...
                            self.centers.append({'name': l, 'Z': q, 'xyz': np.array(
                                [fortran_float(x), fortran_float(y), fortran_float(z)]) * unit})
                    self.geomcenter = (np.amin([c['xyz'] for c in self.centers], axis=0) + np.amax(
                        [c['xyz'] for c in self.centers], axis=0)) / 2
    ...
    def dens(self, x, y, z, cache=None, precomp=None, mask=None, spin=False, trans=False, callback=None,
             interrupt=False):
                        ...
                        if (abs(occup) > self.eps):
                            if (compute):
                                if (callback is not None):
                                    num += 1
                                    if (len(do_list) > 1):
                                        callback('Computing: {0}/{1} (chunk {2}/{3}) ...'.format(num, total, chunk + 1,
                                                                                                 len(do_list)))
                                    else:
                                        callback('Computing: {0}/{1} ...'.format(num, total))
                                if (trans):
                                    dens[start:start + chunk_size] += occup * (
                                                self.mo(ii, x_, y_, z_, 'a', cache, interrupt=interrupt) *
                                                self.mo(ii, x_, y_, z_, 'b', cache, interrupt=interrupt))
                                else:
                                    dens[start:start + chunk_size] += occup * self.mo(ii, x_, y_, z_, s, cache,
            ...
            if ((i == 0) or (i == n[0] - 1)):
                ...
                if (abs(g[0, 1]) > 0):
                    for j in range(1, n[1] - 1):
                        data[i, j, :] += (field[i - 1, j - 1, :] + field[i + 1, j + 1, :] - field[i - 1, j + 1,
                                                                                            :] - field[i + 1, j - 1,
                                                                                                 :]) * g[0, 1] / 2
                if (abs(g[0, 2]) > 0):
                    for k in range(1, n[2] - 1):
                        data[i, :, k] += (field[i - 1, :, k - 1] + field[i + 1, :, k + 1] - field[i - 1, :,
                                                                                            k + 1] - field[i + 1, :,
                                                                                                     k - 1]) * g[
                                             0, 2] / 2
        for j in range(n[1]):
            if ((j == 0) or (j == n[1] - 1)):
                ...
                if (abs(g[1, 2]) > 0):
                    for k in range(1, n[2] - 1):
                        data[:, j, k] += (field[:, j - 1, k - 1] + field[:, j + 1, k + 1] - field[:, j - 1,
                                                                                            k + 1] - field[:, j + 1,
                                                                                                     k - 1]) * g[
                                             1, 2] / 2
        for k in range(n[2]):
    ...
    def _c_sph(self, l, m, lx, ly, lz):
        ...
        for i in range((l - am) // 2 + 1):
            c += self._binom(l, i) * self._binom(i, j) * Fraction(np.math.factorial(2 * l - 2 * i),
                                                                  np.math.factorial(l - am - 2 * i)) * (-1) ** i
        if (c == 0):
            return Fraction(0, 1)
        c_sph = c
        c = 0
    ...
    def write_hdf5(self, filename):
        ...
        with h5py.File(self.h5file, 'r') as fi:
            ...
            for d in ['CENTER_LABELS', 'CENTER_CHARGES', 'CENTER_COORDINATES', 'BASIS_FUNCTION_IDS',
                      'DESYM_CENTER_LABELS', 'DESYM_CENTER_CHARGES', 'DESYM_CENTER_COORDINATES',
                      'DESYM_BASIS_FUNCTION_IDS', 'DESYM_MATRIX',
...
class Grid(object):
    ...
    def read_cube_header(self):
        ...
        with open(self.file, 'rb') as f:
            ...
            for i in range(abs(num)):
                ...
                q = min(maxZ, max(0, int(q)))
                self.centers.append({'name': '{0}'.format(i), 'Z': q,
                                     'xyz': np.array([fortran_float(x), fortran_float(y), fortran_float(z)])})
            self.geomcenter = (np.amin([c['xyz'] for c in self.centers], axis=0) + np.amax(
                [c['xyz'] for c in self.centers], axis=0)) / 2
            ...
            if (num < 0):
                ...
                self.MO = [{'label': '{0}: {1}'.format(i, title), 'ene': 0.0, 'occup': 0.0, 'type': '?', 'sym': 'z'} for
                           i in data]
            else:
                self.nMO = 1
    ...
    def read_grid_header(self):
        with open(self.file, 'rb') as f:
            ...
            for i in range(num):
                ...
                self.centers.append({'name': l, 'Z': name_to_Z(l),
                                     'xyz': np.array([fortran_float(x), fortran_float(y), fortran_float(z)])})
            self.geomcenter = (np.amin([c['xyz'] for c in self.centers], axis=0) + np.amax(
                [c['xyz'] for c in self.centers], axis=0)) / 2
            ...
            for i in range(self.nMO):
                ...
                if (match):
                    self.MO.append({'ene': fortran_float(match.group(3)), 'occup': fortran_float(match.group(4)),
                                    'type': match.group(5).upper(), 'sym': match.group(1), 'num': int(match.group(2)),
                                    'idx': i})
                    ...
                    self.MO.append(
                        {'label': name, 'ene': -np.inf, 'occup': 0.0, 'type': '?', 'sym': 'z', 'num': 0, 'idx': i})
    ...
    def read_luscus_header(self):
        ...
        with open(self.file, 'rb') as f:
            ...
            for i in range(num):
                l, x, y, z = str(f.readline().decode('ascii')).split()
                self.centers.append({'name': l, 'Z': name_to_Z(l), 'xyz': np.array(
                    [fortran_float(x), fortran_float(y), fortran_float(z)]) * angstrom})
            self.geomcenter = (np.amin([c['xyz'] for c in self.centers], axis=0) + np.amax(
                [c['xyz'] for c in self.centers], axis=0)) / 2
            # Read number of orbitals and block size
            f.readline()
            ...
            for i in range(self.nMO):
                name = str(f.readline().decode('ascii'))
                match = re.match(
                    r'\s*GridName=\s*(.+)\s*sym=\s*(\d+)\s*index=\s*(\d+)\s*Energ=\s*(.+)\s*occ=\s*(.+)\s*type=\s*(\w)\s*',
                    name)
                if (match):
                    self.MO.append({'ene': fortran_float(match.group(4)), 'occup': fortran_float(match.group(5)),
                                    'type': match.group(6).upper(), 'sym': match.group(2), 'num': int(match.group(3)),
                                    'idx': i})
                    if (self.MO[-1]['sym'] not in self.irrep):
                        self.irrep.append(self.MO[-1]['sym'])
                else:
    ...
    def run(self):
        ...
        if (orb is None):
            ...
            self.data = self.parent().orbitals.dens(x, y, z, self.cache, precomp=precomp,
                                                    mask=mask, spin=a_and_b, trans=trans, callback=print_func,
                                                    interrupt=self.parent().interrupt)
        elif (orb == -2):
            ngrid = self.parent().xyz.GetInput().GetDimensions()
            data = self.parent().orbitals.dens(x, y, z, self.cache, precomp=precomp,
                                               mask=mask, callback=print_func,
                                               interrupt=self.parent().interrupt).reshape(ngrid[::-1])
...
class LineEditWithButton(QLineEdit):
    def __init__(self, *args, **kwargs):
        ...
        self.setStyleSheet(
            'QLineEdit {{padding-right: {0}px;}}'.format(self.button.sizeHint().width() + frameWidth + 1))
...
class TakeScreenshot(QDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fileBox.setWhatsThis(
            'Type the filename for the saved image, or select it using the button to the right. The file. The file will be overwritten.')
        self.fileButton.setToolTip('Select saved file')
        self.fileButton.setWhatsThis(
            'Select the file to save the image in, or type the filename in the box to the left. The file. The file will be overwritten.')
        ...
        self.panelBox.setWhatsThis(
            'If checked, the text panel with orbital information will be included in the saved image.')
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.setGeometry(
                QStyle.alignedRect(Qt.LeftToRight, Qt.AlignCenter, self.size(), screen.availableGeometry()))
    ...
    def init_UI(self):
        ...
        self.collapseButton.setWhatsThis(
            'Toggle between the collapsed and expanded views of the options panel.<br>Key: <b>Ctrl+_</b>')
        self.densityTypeButton.setToolTip('Select type of density for natural orbitals')
        self.densityTypeButton.setWhatsThis(
            'Select the type of density to compute natural orbitals for, out of those available.<br>Keys: <b>Alt+PgUp</b>, <b>Alt+PgDown</b>')
        self.rootButton.setToolTip('Select root for natural active orbitals')
        self.rootButton.setWhatsThis(
            'Select a root to compute the natural active orbitals for, if the file contains root-specific density matrices.<br>Keys: <b>Shift+PgUp</b>, <b>Shift+PgDown</b>')
        self.irrepButton.setToolTip('Select irrep for the orbital list')
        self.irrepButton.setWhatsThis(
            'This list shows the irreps available in the file, by name or number. Selecting one irrep restricts the orbitals available in the button on the right to the selected irrep. Select "All" for no restriction.<br>Keys: <b>Ctrl+PgUp</b>, <b>Ctrl+PgDown</b>')
        self.orbitalButton.setToolTip('Select orbital to display')
        self.orbitalButton.setWhatsThis(
            'This shows all the orbitals available in the file, belonging to the selected irrep and spin if applicable. If no irrep is selected ("All") and if the file is not a precomputed grid, the electron density, the spin density and the Laplacian of the electron density may also be available. Selecting an orbital displays it in the 3D view above.<br>Keys: <b>PgUp</b>, <b>PgDown</b>')
        self.spinButton.setToolTip('Select spin for the orbital list')
        self.spinButton.setWhatsThis(
            'Select alpha or beta orbitals. This list is only visible if the file contains spin-orbitals.<br>Keys: <b>A</b>, <b>B</b>')
        self.sortedBox.setToolTip('Sort orbitals by occupation & energy')
        self.sortedBox.setWhatsThis(
            'Sort the list of orbitals according to the occupation and energy values. The index still refers to the original order.<br>Key: <b>Shift+S</b>')
        self.listButton.setToolTip('Show/hide full list of orbitals')
        self.listButton.setWhatsThis(
            'Open or close a window showing the list of all orbitals (no restrictions), where custom notes can be added.<br>Key: <b>Shift+L</b>')
        activeFrame.setWhatsThis('RAS1, RAS2 and RAS3 orbitals count as "active"')
        self.frozenButton.setToolTip('Set orbital as frozen')
        self.frozenButton.setWhatsThis(
            'Set the type of the current orbital to "Frozen". This type will be saved in HDF5 or InpOrb files.<br>Key: <b>F</b>')
        self.inactiveButton.setToolTip('Set orbital as inactive')
        self.inactiveButton.setWhatsThis(
            'Set the type of the current orbital to "Inactive". This type will be saved in HDF5 or InpOrb files.<br>Key: <b>I</b>')
        self.RAS1Button.setToolTip('Set orbital as RAS1 (active)')
        self.RAS1Button.setWhatsThis(
            'Set the type of the current orbital to "RAS1" (active). This type will be saved in HDF5 or InpOrb files.<br>Key: <b>1</b>')
        self.RAS2Button.setToolTip('Set orbital as RAS2 (active)')
        self.RAS2Button.setWhatsThis(
            'Set the type of the current orbital to "RAS2" (active). This type will be saved in HDF5 or InpOrb files.<br>Key: <b>2</b>')
        self.RAS3Button.setToolTip('Set orbital as RAS3 (active)')
        self.RAS3Button.setWhatsThis(
            'Set the type of the current orbital to "RAS3" (active). This type will be saved in HDF5 or InpOrb files.<br>Key: <b>3</b>')
        self.secondaryButton.setToolTip('Set orbital as secondary')
        self.secondaryButton.setWhatsThis(
            'Set the type of the current orbital to "Secondary". This type will be saved in HDF5 or InpOrb files.<br>Key: <b>S</b>')
        self.deletedButton.setToolTip('Set orbital as deleted')
        self.deletedButton.setWhatsThis(
            'Set the type of the current orbital to "Deleted". This type will be saved in HDF5 or InpOrb files.<br>Key: <b>D</b>')
        self.resetButton.setToolTip('Reset orbital to its initial type')
        self.resetButton.setWhatsThis(
            'Discard changes to the current orbital type and restore the type (if any) specified in the file.<br>Key: <b>0</b> (zero)')
        self.isovalueSlider.setToolTip('Set value for the isosurfaces (inverse log scale)')
        self.isovalueSlider.setWhatsThis(
            'Change the value for which the isosurfaces are computed. The scale of the slider is inverse logarithmic, moving it to the right makes the value smaller and the surfaces typically "larger". Both positive and negative surfaces are controlled with the slider. To show the isosurface for a value of 0 enable "Nodes" below.<br>Keys: <b>(Shift+)+</b>, <b>(Shift+)-</b>')
        self.isovalueBox.setToolTip('Set value for the isosurfaces')
        self.isovalueBox.setWhatsThis(
            'Value for which the isosurfaces are computed. Lower values make the surfaces typically "larger". Both positive and negative surfaces are affected by this value. To show the isosurface for a value of 0 enable "Nodes" below.')
        self.opacitySlider.setToolTip('Set opacity of the isosurfaces')
        self.opacitySlider.setWhatsThis(
            'Change the opacity of the isosurfaces. Lower opacity makes the surface more transparent.<br>Keys: <b>(Shift+)O</b> (oh), <b>(Shift+)T</b>')
        ...
        self.textureButton.setWhatsThis(
            'Open or close a window where isosurface colors and texture/shading settings can be changed.<br>Key: <b>X</b>')
        self.surfaceBox.setToolTip('Show/hide the isosurfaces')
        self.surfaceBox.setWhatsThis(
            'If checked, the isosurfaces of the current orbital or density are displayed.<br>Key: <b>Ctrl+S</b>')
        self.signButton.setToolTip('Set which parts of the isosurfaces are displayed')
        self.signButton.setWhatsThis(
            'Controls whether the positive, negative or both parts of the isosurface are displayed.<br>Key: <b>Ctrl+Shift+PgUp</b>, <b>Ctrl+Shift+PgDown</b></b>')
        self.nodesBox.setToolTip('Show/hide the nodal surfaces')
        self.nodesBox.setWhatsThis(
            'If checked, the nodal surfaces (value=0) of the current orbital or density are displayed.<br>Key: <b>Ctrl+N</b>')
        self.moleculeButton.setToolTip('Set molecule representation')
        self.moleculeButton.setWhatsThis(
            'Select the type of representation used for the molecule: balls and sticks, smaller nuclei, or none.<br>Key: <b>Ctrl+A</b>')
        self.namesBox.setToolTip('Show/hide the atom names')
        self.namesBox.setWhatsThis(
            'If checked, text labels with the available names are shown at the nucleus or center positions.<br>Key: <b>Ctrl+M</b>')
        ...
        self.boxSizeBox.setWhatsThis(
            'Dimension of the grid box along the x, y and z axes, in bohr. Values can be separated by commas or spaces. Only available if the file is not a precomputed grid.')
        self.boxSizeBox.button.setToolTip('Fit the box size to the molecule')
        self.boxSizeBox.button.setWhatsThis(
            'Set an automatic box size to fit the molecule, considering the current transformation and some extra spacing.')
        self.transformButton.setToolTip('Show/hide the transformation matrix for the box')
        self.transformButton.setWhatsThis(
            'Open or close a window where a translation and transformation matrix can be specified for the grid box.<br>Key: <b>Ctrl+T</b>')
        self.gridPointsBox.setToolTip('Set the number of grid points in the largest dimension')
        self.gridPointsBox.setWhatsThis(
            'Number of grid points along the largest dimension, the number in the other dimensions is adjusted to keep the grid approximately cubic. Only available if the file is not a precomputed grid.')
        self.gradientBox.setToolTip('Show/hide the gradient stream lines (slow)')
        self.gradientBox.setWhatsThis(
            'If checked, stream lines representing the gradient field of the current orbital or density are displayed. Note that this may be slow.<br>Key: <b>Ctrl+G</b>')
        self.showGradientButton.setToolTip('Show/hide advanced options for gradient lines')
        self.showGradientButton.setWhatsThis(
            'Show or hide additional options for the representation of the gradient stream lines.<br>Key: <b>Ctrl+Shift+G</b>')
        self.lineDensityBox.setToolTip('Set the density of gradient lines')
        self.lineDensityBox.setWhatsThis(
            'Controls the number of lines generated. The number of starting points is the square of this number for each nucleus or center.')
        self.startRadiusBox.setToolTip('Set the starting radius for the gradient lines')
        self.startRadiusBox.setWhatsThis(
            'The lines start on a sphere of this radius (in bohr) around each nucleus or center.')
        self.maxStepsBox.setToolTip('Set the maximum number of steps for the gradient lines')
        self.maxStepsBox.setWhatsThis(
            'Maximum number of steps in the integration of the gradient stream lines. Reduce to speed up.')
        self.downButton.setToolTip('Show only lines towards lower values')
        self.downButton.setWhatsThis(
            'Integrate the gradient stream lines only towards lower values from the starting points (outwards for the electron density).<br>Key: <b>Ctrl+Shift+D</b>')
        self.bothButton.setToolTip('Show lines in both directions')
        self.downButton.setWhatsThis(
            'Integrate the gradient stream lines in both directions.<br>Key: <b>Ctrl+Shift+B</b>')
        self.upButton.setToolTip('Show only lines towards higher values')
        self.upButton.setWhatsThis(
            'Integrate the gradient stream lines only towards higher values from the starting points (inwards for the electron density).<br>Key: <b>Ctrl+Shift+U</b>')
        self.cancelButton.setToolTip('Interrupt current calculation')
        self.cancelButton.setWhatsThis(
            'Interrupt the current calculation as soon as possible, the display window will show the partial result in pink.<br>Key: <b>Esc</b>')
        ...
        self.typeGroup = group_widgets(self.frozenButton, self.inactiveButton, activeFrame, self.secondaryButton,
                                       self.deletedButton, spacing=6)
        ...
        self.optionsDock.setWhatsThis(
            'This panel contains the main options for the display window. The panel can be collapsed, or detached by dragging the top part, to increase the size of the viewing area.')
        ...
        self.listDock.hide()
        self.listDock.setWhatsThis(
            'This is a detachable window that shows all orbitals in the file. For each orbital you can add a custom note and specify whether it should be included in the electron and spin density. The notes are saved in HDF5 format.<br>Key: <b>Ctrl+L</b>')
        ...
        self.transformDock.setWhatsThis(
            'This is a detachable window that allows setting a transformation (rotation, scaling, shearing, translation) for the grid box. This is only possible if the current file is not a precomputed grid. The transformation affects the display and any grid saved in the cube format.')
        ...
        self.textureDock.setWhatsThis(
            'This is a detachable window that allows modifying the color and texture/shading properties for the isosurfaces.')
    ...
    def init_VTK(self):
        ...
        self.vtkWidget.setWhatsThis(
            'Render window where the orbitals are displayed. Click and drag with the primary (left) mouse button to rotate; use the secondary (right) button, the wheel, or Ctrl+Shift+primary to zoom; the tertiary (middle) button or Shift+primary to translate; Ctrl+primary to rotate in the screen plane.')
    ...
    def MO(self, value):
        try:
            ...
            pass
        self._MO = value
        if ((value is None) or (old_orb is None) or (self.orbital >= len(self.MO)) or not np.allclose(old_orb, self.MO[
            self.orbital - 1]['coeff'])):
            self._tainted = True
    ...
    def _isovalue_changed(self, new, old):
        ...
        slider_value = round(
            self.isovalueSlider.maximum() - reldist * (self.isovalueSlider.maximum() + self.isovalueSlider.minimum()))
    ...
    def _opacity_changed(self, new, old):
        if (new == old):
            return
        slider_value = round(
            self.opacitySlider.minimum() + new * (self.opacitySlider.maximum() + self.opacitySlider.minimum()))
    ...
    def populate_orbitals(self):
        ...
        if (self.irrep == 'All'):
            ...
            orbidx = {
                i + 1: [-snap(o['occup']), -np.inf if np.isnan(o['ene']) else o['ene'], np.copysign(1, o['occup'])]
            ...
            if ((not self.isGrid) and any([(o['occup'] != 0.0) for o in self.MO])):
                ...
                elif ((self.dens == 'Difference') or (
                        (self.dens == 'WFA') and ('_NDO' in self.rootButton.currentText()))):
            ...
            orblist = {i + 1: self.orb_to_list(i + 1, o) for i, o in enumerate(self.MO) if
                       ((o['sym'] == self.irrep) and not o.get('hide'))}
            orbidx = {
                i + 1: [-snap(o['occup']), -np.inf if np.isnan(o['ene']) else o['ene'], np.copysign(1, o['occup']),
                        o['sym']]
    ...
    def default_box(self, clearance=4.0):
        ...
        xyz = np.array(
            [c['xyz'] for c in self.orbitals.centers if (not isEmpty(c.get('basis', [0])))]) - self.orbitals.geomcenter
    ...
    def update_cache(self, ngrid):
        ...
        if (self.isGrid):
            ...
            for i in ['float64', mintype]:
                ...
                if (npoints < 100):
                    return
            self._cache_file = np.memmap(os.path.join(self._tmpdir, '{0}.cache'.format(__name__.lower())), dtype=dtype,
                                         mode='w+', shape=(sum(self.orbitals.N_bas), npoints))
            ...
            if (maxdens > 0):
                self._dens_cache = np.memmap(os.path.join(self._tmpdir, '{0}.dens_cache'.format(__name__.lower())),
                                             dtype=dtype, mode='w+', shape=(maxdens, npoints + 1))
    ...
    def config_scratch(self):
        ...
        dialog.sizeBox.setWhatsThis(
            'The maximum size for the scratch file, common units are accepted: KB, KiB, MB, MiB, GB, GiB, TB, TiB.')
        dialog.scratchBox.setToolTip('Use a scratch file for caching basis functions (faster, but uses disk space)')
        dialog.scratchBox.setWhatsThis(
            'Enable or disable the usage of a scratch file. Switching orbitals and computing densities is considerably faster with a scratch file.')
        dialog.tmpLabel.setToolTip('Location of the scratch file for the current instance')
        dialog.tmpLabel.setWhatsThis(
            'The scratch file, if enabled, will be created in this directory, and removed when quitting.')
        dialog.stringLabel.setToolTip('Maximum, current, recommended sizes')
        dialog.stringLabel.setWhatsThis(
            'Maximum scratch size, current scratch usage, and recommended scratch size for the current view.')
        dialog.exec_()
    ...
    def build_surface(self):
        ...
        self._computeVolumeThread.disable_list = [self.setScratchAction, self.densityTypeGroup, self.rootGroup,
                                                  self.irrepGroup, self.orbitalGroup, self.boxSizeGroup,
                                                  self.gridPointsGroup, self.fitBoxAction, self.gradientBox,
                                                  self.gradientGroup, self.transformDock]
    ...
    def volume_computed(self):
        ...
        try:
            ...
            vtkmo = numpy_support.numpy_to_vtk(1e-6 * (np.random.rand(points.GetNumberOfPoints()) - 0.5), 1,
                                               vtk.VTK_DOUBLE)
    ...
    def isovalueSlider_changed(self, value):
        logrange = np.log(self._maxval / self._minval)
        reldist = (value - self.isovalueSlider.minimum()) / (
                    self.isovalueSlider.maximum() - self.isovalueSlider.minimum())
    ...
    def set_panel(self):
        ...
        try:
            if (len(self.MO) == len(self.orbitals.base_MO[0])):
                modified = not np.allclose(self.MO[self.orbital - 1]['coeff'],
                                           self.orbitals.base_MO[0][self.orbital - 1]['coeff'])
            else:
                modified = True
        ...
        if (self.rootGroup.isEnabled()):
            ...
            if (len(self.MO) == 0):
                ...
                text = 'Difference from {0} {1} to {2}\n'.format(state.lower(),
                                                                 *self.rootButton.currentText().split(u' → '))
        ...
        if (self.orbital is None):
            ...
            if (self.orbital is not None):
                ...
                if ('label' in orb):
                    ...
                    text += '#{0}{1}   E: {2:.6f}   occ: {3:.4f}   {4}'.format(self.orbital, sym, orb['ene'],
                                                                               orb['occup'], tp)
        # Update the counts
        ...
        for o in self.MO:
            try:
                ...
                pass
        text += '\n' + '   '.join(
            ['{0}: {1}'.format(i, ','.join(map(str, types[i]))) for i in ['F', 'I', '1', '2', '3', 'S', 'D'] if
             (sum(types[i]) > 0)])
    ...
    def overwrite(self):
        ...
        if (QMessageBox.question(self, 'Overwrite?', message, QMessageBox.Yes | QMessageBox.No,
    ...
    def show_about(self):
        ...
        QMessageBox.about(self, 'About {0}'.format(__name__),
                      ...
                      '''.format(__name__, __version__, __copyright__, __author__, python_version, QtVersion,
                                 vtk_version, self.settings.fileName())
                          )

    def show_screenshot(self):
        if (self.screenshot is None):
            self.screenshot = TakeScreenshot(self)
            self.screenshot.transparentBox.setChecked(
                qt_to_bool(self.settings.value('Screenshots/transparent', 'true')))
...
class ListDock(QDockWidget):
    ...
    def init_UI(self):
        ...
        self.activeButton.setWhatsThis(
            'Select only occupied active orbitals when computing the electron and spin density.')
    ...
    def select_active(self):
        ...
        if (self.parent().orbitals.MO_b):
            ...
            alphaMO = self.parent().orbitals.MO
        for i, o in zip(self.orbCheckBoxes,
                        [j for i in zip_longest(alphaMO, self.parent().orbitals.MO_b) for j in i if (j is not None)]):
            if (i.isEnabled()):
                tp = o['type']
                if ('newtype' in o):
                    tp = o['newtype']
...
class TransformDock(QDockWidget):
    ...
    def init_UI(self):
        ...
        self.applyButton.setWhatsThis(
            'By clicking this button the transformation defined here is applied to the grid box and the display is updated.')
        self.alignButton.setToolTip('Align box axes with the molecule')
        self.alignButton.setWhatsThis(
            'Set the transformation to align the box axes with the principal axes of the molecule')
...
class TextureDock(QDockWidget):
    ...
    def init_UI(self):
        ...
        self.ambientSlider.setWhatsThis(
            'This gives the amount of the surface color that will show up regardless of its orientation with respect to the light sources.')
        ...
        self.diffuseSlider.setWhatsThis(
            'This gives the amount of surface color due to diffuse light reflection, it is brighter where the surface faces the light sources.')
        ...
        self.powerSlider.setWhatsThis(
            'This controls the size of the highlights, the higher the power the smaller and brighter the highlights.')
        ...
        self.transparencySlider.setWhatsThis(
            'This controls the transparency of the surface, unlike the main "opacity" slider, it does not affect the highlights.')
        ...
        self.fresnelSlider.setWhatsThis(
            'This controls the angle-dependent transparency, positive values make the "edges" more opaque, negative values make the "center" more opaque.')
        ...
        self.interpolationButton.setWhatsThis(
            'Selects between different types on interpolation for the surface normal: flat (no interpolation), Gouraud and Phong (both give the same smooth result).')
        self.representationButton.setToolTip('Choose the type of representation for the isosurface')
        self.representationButton.setWhatsThis(
            'Selects between different types of representation: points, wireframe or surface.')
        self.sizeBox.setToolTip('Point or line width for points and wireframe representations')
        self.sizeBox.setWhatsThis(
            'Size for the points in the point representation or width for the lines in wireframe representation.')
        self.negColorButton.setToolTip('Color used for the negative parts of the isosurfaces')
        self.negColorButton.setWhatsThis(
            'When an isosurface has (or can have) both negative and positive parts, the negative parts are drawn with this color.')
        self.zeroColorButton.setToolTip('Color used for unsigned isosurfaces (density)')
        self.zeroColorButton.setWhatsThis(
            'When an isosurface is unsigned (the electron density), it is drawn with this color.')
        self.posColorButton.setToolTip('Color used for the positive parts of the isosurfaces')
        self.posColorButton.setWhatsThis(
            'When an isosurface has (or can have) both negative and positive parts, the positive parts are drawn with this color.')
        self.specColorButton.setToolTip('Color of the highlights')
        self.specColorButton.setWhatsThis(
            'This is the color used for the specular highlights. Ambient and diffuse use the surface color.')
        ...
        self.GBSButton.setWhatsThis(
            'Loads a preset with gold color for positive, bronze color for negative and silver color for unsigned.')
        ...
        self.CMYButton.setWhatsThis(
            'Loads a preset with yellow for positive, magenta for negative and cyan for unsigned.')
    ...
    def _ambient_changed(self, new, old):
        if (new == old):
            return
        slider_value = round(
            self.ambientSlider.minimum() + new * (self.ambientSlider.maximum() + self.ambientSlider.minimum()))
    ...
    def _diffuse_changed(self, new, old):
        if (new == old):
            return
        slider_value = round(
            self.diffuseSlider.minimum() + new * (self.diffuseSlider.maximum() + self.diffuseSlider.minimum()))
    ...
    def _specular_changed(self, new, old):
        if (new == old):
            return
        slider_value = round(
            self.specularSlider.minimum() + new * (self.specularSlider.maximum() + self.specularSlider.minimum()))
    ...
    def _power_changed(self, new, old):
        ...
        slider_value = round(
            self.powerSlider.minimum() + reldist * (self.powerSlider.maximum() + self.powerSlider.minimum()))
    ...
    def _transparency_changed(self, new, old):
        if (new == old):
            return
        slider_value = round(self.transparencySlider.minimum() + new * (
                    self.transparencySlider.maximum() + self.transparencySlider.minimum()))
    ...
    def transparencySlider_changed(self, value):
        new = (value - self.transparencySlider.minimum()) / (
                    self.transparencySlider.maximum() - self.transparencySlider.minimum())
    ...
    def _fresnel_changed(self, new, old):
        ...
        reldist = (new + 2) / 4
        slider_value = round(
            self.fresnelSlider.minimum() + reldist * (self.fresnelSlider.maximum() + self.fresnelSlider.minimum()))
    ...
    def fresnelSlider_changed(self, value):
        new = 4 * (value - self.fresnelSlider.minimum()) / (
                    self.fresnelSlider.maximum() - self.fresnelSlider.minimum()) - 2
...
class BGColorDialog(QDialog):
    ...
    def init_UI(self):
        ...
        self.mainColorButton.setWhatsThis(
            'Choose background color for densities, orbitals of uknown/mixed type, or if color by type is disabled')
        ...
        self.FColorButton.setWhatsThis(
            'Choose background color when displaying frozen orbitals (if color by type is enabled)')
        self.IColorButton.setToolTip('Background color for inactive orbitals')
        self.IColorButton.setWhatsThis(
            'Choose background color when displaying inactive orbitals (if color by type is enabled)')
        self.R1ColorButton.setToolTip('Background color for RAS 1 orbitals')
        self.R1ColorButton.setWhatsThis(
            'Choose background color when displaying RAS 1 orbitals (if color by type is enabled)')
        self.R2ColorButton.setToolTip('Background color for RAS 2 orbitals')
        self.R2ColorButton.setWhatsThis(
            'Choose background color when displaying RAS 2 orbitals (if color by type is enabled)')
        self.R3ColorButton.setToolTip('Background color for RAS 3 orbitals')
        self.R3ColorButton.setWhatsThis(
            'Choose background color when displaying RAS 3 orbitals (if color by type is enabled)')
        self.SColorButton.setToolTip('Background color for secondary orbitals')
        self.SColorButton.setWhatsThis(
            'Choose background color when displaying secondary orbitals (if color by type is enabled)')
        self.DColorButton.setToolTip('Background color for deleted orbitals')
        self.DColorButton.setWhatsThis(
            'Choose background color when displaying deleted orbitals (if color by type is enabled)')
