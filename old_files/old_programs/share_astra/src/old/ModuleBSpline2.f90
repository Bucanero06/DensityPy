...
!!$     DoublePrecision, private, allocatable :: vec(:)
   ...
   contains
     ...
     !!       \frac{d^{n_{2}}Bs_{j}(r)}{dr^{n_{2}}}
     !! \f]
     procedure :: Integral          =>  BSplineIntegral
...
!!$     procedure :: CauchyIntegral    =>  BSplineCauchyIntegral
     ...
     !> Checks whether the B-splines characteristics (fingerprint) saved 
     !! in form of a class, coincides or not with the current B-splines in used.
     procedure :: MatchFingerPrint  =>  BSplineMatchFingerPrint

     !> Checks whether the B-splines characteristics (fingerprint) saved 
     !! in a unit, coincides or not with the current B-splines in used.
     procedure :: MatchFingerPrintOnUnit =>  BSplineMatchFingerPrintOnUnit

     !> Writes in a unit the adequate B-splinesradial basis information 
     !! (fingerprint) that allows to identify it from other 
     !! B-splines sets that could possible be used.
     procedure :: WriteFingerPrint  =>  BSplineWriteFingerPrint
  ...
  !> Defines a fingerprint for a B-spline basis.
  type, public :: ClassBSplineFingerPrint
     !
     ! {{{ Private Attributes

     !> B-splines number of nodes.
     integer        , private              :: n
     !> B-splines order.
     integer        , private              :: Order
     !> B-splines nodes grid.
     Real(kind(1d0)), private, allocatable :: grid(:)

     ! }}}
   contains
     !
     !> Frees ClassBSplineFingerPrint objects.
     procedure :: Free   =>  FingerPrintFree
     !> Writes the B-splines fingerprint in a unit.
     procedure :: Write  =>  FingerPrintWrite
     !> Reads the B-splines fingerprint from a unit.
     procedure :: Read   =>  FingerPrintRead
     !> Initializes ClassBSplineFingerPrint.
     procedure :: Init   =>  FingerPrintInit
     !> Extract the B-splines fingerprint from a ClassBSpline.
     generic   :: assignment(=) => ExtractBSplineFingerPrint
     procedure, private :: ExtractBSplineFingerprint
     !
  end type ClassBSplineFingerPrint


  interface 
     Pure DoublePrecision function D2DFun( x ) 
...
contains
  ...
  ! {{{ BSplineFingerPrint Methods 

  !> Writes the B-splines fingerprint in a unit.
  subroutine FingerPrintWrite(FingerPrint,OutputUnit)
    Class(ClassBSplineFingerPrint), intent(in) :: FingerPrint
    integer               , intent(in) :: OutputUnit
    integer :: i
    character(len=100) :: nstrn, GridFormat
    ...
    if( UnitIsWritable=="NO" .or. .not. UnitIsOpen )call Assert("Invalid unit")

    if( IsUnitFormatted == "YES" )then
       !
       write(OutputUnit,"(2i)") FingerPrint%n,FingerPrint%Order
       write(nstrn,*)FingerPrint%n
       GridFormat="("//trim(adjustl(nstrn))//"(d24.16))"
       write(OutputUnit,GridFormat) (FingerPrint%Grid(i),i=0,FingerPrint%n-1)
       !
    elseif( IsUnitFormatted == "NO" )then
       !
       write(OutputUnit)FingerPrint%n,FingerPrint%Order
       write(OutputUnit)(FingerPrint%Grid(i),i=0,FingerPrint%n-1)
       !
    else
       call Assert("Invalid format")
    endif
  ...
  subroutine FingerPrintRead(FingerPrint,InputUnit,iostat)
    ...
    if( UnitIsReadable=="NO" )then
       ...
       call Assert("Unreadable Unit")
    endif

    if( IsUnitFormatted == "YES" )then
       !
       read(InputUnit,"(2i)",iostat=iostat_,iomsg=iomsg) NumberNodes, BsplineOrder
       if(iostat_/=0)then
          if(present(iostat))then
             iostat=iostat_
             return
       ...
       call FingerPrint%Init( NumberNodes, BsplineOrder )
       read(InputUnit,"(*(d24.16))",iostat=iostat_,iomsg=iomsg) (FingerPrint%Grid(i),i=0,FingerPrint%n-1)
       if(iostat_/=0)then
          if(present(iostat))then
             iostat=iostat_
             return
    ...
    elseif( IsUnitFormatted == "NO" )then
       !
       read(InputUnit,iostat=iostat_,iomsg=iomsg) NumberNodes, BsplineOrder
       if(iostat_/=0)then
          if(present(iostat))then
             iostat=iostat_
             return
       ...
       call FingerPrint%Init( NumberNodes, BsplineOrder )
       read(InputUnit,iostat=iostat_,iomsg=iomsg)(FingerPrint%Grid(i),i=0,FingerPrint%n-1)
       if(iostat_/=0)then
          if(present(iostat))then
             iostat=iostat_
             return
  ...
  !> Initializes ClassBSplineFingerPrint.
  subroutine FingerPrintInit( FingerPrint, NumberNodes, BsplineOrder )
    Class(ClassBSplineFingerPrint), intent(inout) :: FingerPrint
    integer               , intent(in)    :: NumberNodes
    integer               , intent(in)    :: BsplineOrder
    FingerPrint%n=NumberNodes
    FingerPrint%Order=BsplineOrder
    if(allocated(Fingerprint%Grid))deallocate(FingerPrint%Grid)
    allocate(FingerPrint%Grid(0:FingerPrint%n-1))
    FingerPrint%Grid=0.d0
  ...
  !> Checks whether tha B-splines characteristics (fingerprint) saved 
  !! previously in a unit, coincides or not with the current B-splines in used.
  logical function BsplineMatchFingerPrint(swav,FingerPrint) result(Match)
    Class( ClassBSpline )      , intent(in) :: swav
    type( ClassBSplineFingerPrint ), intent(in) :: FingerPrint
    integer :: i
    DoublePrecision, parameter :: RELATIVE_TOLERANCE=1.d-12
    DoublePrecision :: NodePositionTolerance
    Match=.false.
    if( FingerPrint%n /= swav%NNodes ) return
    if( FingerPrint%Order /= swav%Order ) return
    do i=0,swav%NNodes-1
       NodePositionTolerance = RELATIVE_TOLERANCE * max(abs(FingerPrint%Grid(i)),abs(swav%Grid(i)))
       if( abs(FingerPrint%Grid(i)-swav%Grid(i)) > NodePositionTolerance )return
    enddo
    Match=.TRUE.
  end function BsplineMatchFingerPrint
  

  logical function BsplineMatchFingerPrintOnUnit(BSplineSet,uid) result(Match)
    Class(ClassBSpline), intent(in) :: BSplineSet
    integer            , intent(in) :: uid
    type(ClassBSplineFingerPrint) :: FingerPrint
    call FingerPrint%Read(uid)
    Match=BSplineSet%MatchFingerprint(FingerPrint)
    call FingerPrint%Free()
  end function BsplineMatchFingerPrintOnUnit
  
  
  !> Extract the B-splines fingerprint from a ClassBSpline.
  subroutine ExtractBsplineFingerPrint( FingerPrint, BSplineSet )
    Class( ClassBSplineFingerPrint ), intent(inout) :: FingerPrint
    Class( ClassBSpline )           , intent(in)    :: BSplineSet
    call FingerPrint%Init(BSplineSet%NNodes,BSplineSet%Order)
    FingerPrint%Grid(0:FingerPrint%n-1)=BSplineSet%Grid(0:BSplineSet%NNodes-1)
  end subroutine ExtractBsplineFingerPrint


  !> Writes in a unit the adequate B-splinesradial basis information (fingerprint) that allows to identify it from other B-splines sets that could possible be used.
  subroutine BSplineWriteFingerprint( BSplineSet, uid )
    Class(ClassBSpline), intent(in) :: BSplineSet
    integer            , intent(in) :: uid
    type(ClassBSplineFingerPrint) :: fingerprint
    fingerprint = BSplineSet
    call fingerprint%write(uid)
    call fingerprint%free
  end subroutine BSplineWriteFingerprint
...
!!$    s%CARFUNINIT= .FALSE.
  ...
  Pure DoublePrecision function Unity(x) result(y)
    DoublePrecision, intent(in) :: x
    y=1.d0
  end function Unity
...
!!$    s%CARFUNINIT=.FALSE.
  ...
  DoublePrecision function BSplineFunctionEval(s,x,fc,k,SKIP_FIRST)
    class(ClassBSpline), intent(in) :: s
    DoublePrecision    , intent(in) :: x
    DoublePrecision    , intent(in) :: fc(:)
    integer, optional  , intent(in) :: k
    logical, optional  , intent(in) :: SKIP_FIRST
    integer :: Bs,i,firstBs
    call CheckInitialization(s)
    BSplineFunctionEval=0.d0
    i=which_interval(x,s)
    if(i<0.or.i>s%NNodes-2)return
    firstBs=1
    if(present(SKIP_FIRST).and.SKIP_FIRST)firstBs=2
    do Bs=max(firstBs,i+1),min(s%NBSplines,i+s%Order)
       BSplineFunctionEval=BSplineFunctionEval+s%Eval(x,Bs,k)*fc(Bs-firstBs+1)
    enddo
    return
  ...
  DoublePrecision function BSplineIntegral( &
    ...
    if( present(BreakPoint) )then
       ...
       !***
!!$       Integral = &
!!$            BSplineDriverIntegral(s,FunPtr,Bs1,Bs2,n1,n2, a, BreakPoint ) + &
!!$            BSplineDriverIntegral(s,FunPtr,Bs1,Bs2,n1,n2, BreakPoint, b )
       Integral = &
            BSplineDriverIntegral(s,FunPtr,Bs1,Bs2,n1,n2, a, BreakPoint ) + &
            BSplineDriverIntegral(s,FunPtr,Bs1,Bs2,n1,n2, NewBreakPoint, b )
    else
       Integral = BSplineDriverIntegral(s,FunPtr,Bs1,Bs2,n1,n2,a,b)
    endif
    !
    return
    !
  ...
  ! }}}
  DoublePrecision function BSplineDriverIntegral(s,FunPtr,Bs1,Bs2,n1,n2,a,b) &
       result( Integral )
    !
  ...
  DoublePrecision function BSplineMonoIntegral( &
    ...
    !***
!!$       Integral = &
!!$            BSplineMonoIntegralDriver(s,FunPtr,Bs1,n1,a,BreakPoint) + &
!!$            BSplineMonoIntegralDriver(s,FunPtr,Bs1,n1,BreakPoint,b)
       Integral = &
            BSplineMonoIntegralDriver(s,FunPtr,Bs1,n1,a,NewBreakPoint) + &
            BSplineMonoIntegralDriver(s,FunPtr,Bs1,n1,NewBreakPoint,b)
    else
       Integral = &
            BSplineMonoIntegralDriver(s,FunPtr,Bs1,n1,a,b)
    endif
    !
  contains
    !
  ...
  !! If the first (last) boundary of the integration interval is not 
  !! specified, then the value \f$a=-\infty\f$ and \f$b=\infty\f$ is assumed.
  DoublePrecision function BSplineMonoIntegralDriver(s,FunPtr,Bs1,n1,a,b) &
       result( Integral )
    !
    Class(ClassBSpline)      , intent(in) :: s
    procedure(D2DFun)        , pointer    :: FunPtr
...
!!$  end function BSplineCauchyIntegral
  ...
  DoublePrecision function MixedBSplineIntegral( &
       s1, s2, FunPtr, Bs1, Bs2, BraDerivativeOrder, KetDerivativeOrder, LowerBound, UpperBound ) &
       result( Integral )
    !
  ...
  contains
    Pure DoublePrecision function MultipolarPotential(x) result(y)
      DoublePrecision, intent(in) :: x
