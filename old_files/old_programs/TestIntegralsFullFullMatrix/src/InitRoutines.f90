...
subroutine InitFromFiles
  ...
  integer             :: i, j, iIrrep, l, m, iexerror
...
subroutine STEX_BASIS(ielement)
  ...
  type(ClassIrrep), pointer     :: StexIrrepPtr
  integer                       :: StexIrrepIndex, syha, syep
...
allocate(iha(1:Ndim),iep(1:Ndim),TotSym(1:Ndim))
iha=0
iep=0
TotSym=0
k=1
...
StexIrrepPtr   => GlobalGroup%GetIrrep("Ag")  !usually, reference configuration is a totally symmetric state (even number of electrons)
StexIrrepIndex =  GlobalGroup%getIrrepIndex(StexIrrepPtr)
TotSym(1)=StexIrrepIndex

nIrreps = Globalgroup%getnIrreps()
allocate(Irreps(1:nIrreps))
Irreps = GlobalGroup%getIrrepList()

!.. This is for the Irrep
!Symmetry(k)=Irreps(ielement(iep(k))%irr)*Irreps(ielement(iha(k))%irr)
k=2
l=0
Do i=1,Nbasis
      ...
      Do j=1,Nbasis
         If(.not.ielement(j)%RCtype)Then
            If(ielement(i)%spin.eq.ielement(j)%spin)Then
               ...
               iha(k)=i
               iep(k)=j
               StexIrrepPtr => Irreps(ielement(iep(k))%irr)*Irreps(ielement(iha(k))%irr)
               TotSym(k) = GlobalGroup%getIrrepIndex(StexIrrepPtr)
               !.. This is for the Irrep
               !Symmetry(k)=IrrepList(ielement(i)*IrrepList(j)
               k=k+1
               ! End IF
            End If
         End IF
